...
This runs every check Phase 3 declares — chronology, manuscript lint (set `project.lint_profile` in `PROJECT_STATE.yaml`, or it falls back to `project.subgenre`/`project.genre`), chapter ordering, and total word count — in one pass, and is the same command `advance-phase` uses to decide whether Phase 3 is done. A non-zero exit means at least one check failed; read `work/gate-report.md` for which one.
